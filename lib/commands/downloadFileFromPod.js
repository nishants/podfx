...
const downloadFileFromPod = (exec, chosenNameSpace, podName, containerName, srcPath, tgtPath) => {
    ...
    exec.exec(
      ...
      async () => {
        ...
        fs.createReadStream(tgtPath).pipe(
          tar.x({
            strip: 1,
            C: require('path').dirname(tgtPath) // alias for cwd:'some-dir', also ok
          })
        );
        resolve();
      },
...
module.exports = downloadFileFromPod;
